...
const DashboardLayout: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const navItems = [
    { name: 'Federation Hub', path: '/', icon: LayoutDashboard },
    { name: 'Identity Providers', path: '/idps', icon: Cloud },
    { name: 'Workload Identity', path: '/workloads', icon: Server },
    { name: 'Federated Access', path: '/access', icon: Fingerprint },
    { name: 'Policy Engine', path: '/policies', icon: ShieldCheck },
    { name: 'Token Exchange', path: '/tokens', icon: Key },
    { name: 'Network Trust', path: '/network', icon: Network },
    { name: 'Conditional Access', path: '/conditional', icon: Lock },
    { name: 'Audit & Compliance', path: '/audit', icon: FileText },
    { name: 'Multi-Tenant View', path: '/tenants', icon: Users },
    ...
    <div className="flex h-screen bg-slate-950 text-slate-200 overflow-hidden font-sans">
      ...
      <aside className={`bg-slate-900 border-r border-slate-800 transition-all duration-300 ${sidebarOpen ? 'w-64' : 'w-20'} flex flex-col`}>
        <div className="p-6 flex items-center gap-4 border-b border-slate-800">
          ...
          {sidebarOpen && <span className="font-bold text-xl tracking-tight bg-gradient-to-r from-white to-cyan-400 bg-clip-text text-transparent">Fed.Cloud</span>}
        </div>
          ...
          <div className="flex items-center gap-3 p-2 bg-slate-800/50 rounded-xl">
            ...
            {sidebarOpen && (
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold truncate">Federation Architect</p>
                <p className="text-xs text-slate-500 truncate">Zero Trust Control</p>
              </div>
            )}
          </div>
        </div>
        ...
        <header className="h-16 bg-slate-900/50 backdrop-blur-md border-b border-slate-800 flex items-center justify-between px-6 z-10">
          <div className="flex items-center gap-4 flex-1">
            ...
            <div className="relative max-w-md w-full">
              ...
              <input 
                type="text" 
                placeholder="Search identity providers, workload IDs, access policies, or audit logs..." 
          ...
          <div className="flex items-center gap-3">
            <div className="flex items-center gap-2 px-3 py-1 bg-cyan-500/10 text-cyan-500 border border-cyan-500/20 rounded-full text-xs font-bold">
              TRUST BOUNDARY: SECURE
            </div>
            <button className="p-2 hover:bg-slate-800 rounded-lg text-slate-400 relative">
              <Bell size={20} />
              <span className="absolute top-2 right-2 w-2 h-2 bg-cyan-600 rounded-full border-2 border-slate-900"></span>
            </button>
          </div>
        </header>
